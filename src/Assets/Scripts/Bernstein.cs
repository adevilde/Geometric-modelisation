...
public class Bernstein : MonoBehaviour
{
    ...
    void buildPolysBernstein() {
        ...
        for(int k=0; k<=n ; ++k) {
            ...
            List<Vector2> ListVectors = new List<Vector2>(); 

            for(int i=0; i<tToEval.Count; ++i) {
                float B = (float) (KparmiN(k,n)*Math.Pow(1-tToEval[i], n-k)*Math.Pow(tToEval[i], k));
                Vector2 v = new Vector2(tToEval[i],B);
                ListVectors.Add(v);
            }

            ListePoints.Add(ListVectors);
        }
    }
