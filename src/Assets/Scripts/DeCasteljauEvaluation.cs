...
public class DeCasteljauEvaluation : MonoBehaviour
{
   ...
   //////////////////////////////////////////////////////////////////////////
    ...
    {
        List<float> Xcur = new List<float>();
        List<float> Ycur = new List<float>();
        ...
        // Boucle principale de l'algorithme : on itere sur les points de controle
        while (n > 1)
        {
            for (int i = 0; i < n - 1; ++i)
            {
                Xcur[i] = (1 - t) * Xcur[i] + t * Xcur[i + 1];
                Ycur[i] = (1 - t) * Ycur[i] + t * Ycur[i + 1];
            }

            n -= 1;
        }
        return new Vector2(Xcur[0], Ycur[0]);
    }
